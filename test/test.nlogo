...
to test
  ...
  show (word "testing responses yes = "
    cbr:yes " no = " cbr:no " equal = "
    cbr:equal " and invalid = "
    cbr:invalid )
  ...
  show (word "testing cbr:time " cbr:get-time case-base some-case-1)
...
;  show (word "cbr:same - " cbr:same case-base some-case-1 some-case-2 some-case-3)
  ...
  show (word "testing new cbr:time " cbr:get-time case-base some-case-1)

  ; Listing

  foreach cbr:all case-base [ this-case ->
    show (word "A case = " this-case)
    show (word "iterate testing cbr:state " cbr:state case-base this-case)
    show (word "iterate testing cbr:state " cbr:state case-base this-case)
    show (word "iterate testing cbr:outcome " cbr:outcome case-base this-case)
    show (word "iterate testing cbr:get-time " cbr:get-time case-base this-case)
  ]
  ...
  cbr:set-earliest case-base 2
  cbr:forget case-base
  show (word "Size of forgetting case-base = " length cbr:all case-base)
  ...
  show (word "Added case 5 gives length = " length cbr:all case-base)
  show (word "testing cbr:decision " cbr:decision case-base some-case-5)
  show (word "testing cbr:state " cbr:state case-base some-case-5)
  show (word "testing cbr:outcome " cbr:outcome case-base some-case-5)
  show (word "testing cbr:time " cbr:get-time case-base some-case-5)

  show (word "case base = " case-base)
...
to-report comparator [some-case-base case-1 case-2 case-3]
  ...
  foreach n-values (length arg-1) [i -> i] [ i ->
    if item i arg-1 = item i arg-3 [
      set comparison-1-and-3 comparison-1-and-3 + 1
    ]
  ]
  let comparison-2-and-3 0
  foreach n-values (length arg-2) [i -> i] [ i ->
    if item i arg-2 = item i arg-3 [
