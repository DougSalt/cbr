...
let fish cbr:add simple-case-base [ "lays eggs" "does not breath air"] "fish" .2
...
; MUST RETURN ONLY cbr:lt cbr:gt cbr:eq cbr:incmp

to-report some-comparator [ some-case-base src-case obj-case ref-case ]

  ;show "We are using a custom comparator..."
  ...
  foreach n-values length ref-state [ i -> i ] [ i ->
    ...
    if item i ref-state = item i obj-state [
...
to bespoke-comparator
  ...
  let some-creature [ "lays eggs" "breathes air" "flies" "stings"]

  ;let result cbr:match simple-case-base some-creature "fish?"
  ;
  let result cbr:match simple-case-base some-creature "bird?"

  show "So we have some creature with the following attributes"
  show (word some-creature "\n")

  show "Is our mystery creature a fish?"
  show cbr:outcome simple-case-base result
  show result

  show "Now let's try again, with our own comparator."
  show "Adding our version of the comparator..."
...
to-report comparator-pattern [ some-case-base src-case obj-case ref-case]
  ; ...
  report cbr:lt
  ; ...
  report cbr:gt
